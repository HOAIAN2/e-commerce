...
import { dbSelectUserByUsername, dbUpdateUserInfo, usersCache, dbUpdateUserAvatar } from "../database/index.js"
...
async function handleEditInfo(request: FastifyRequest, reply: FastifyReply) {
    ...
    try {
        const user = await dbSelectUserByUsername(tokenData.username)
        if (!user) return reply.code(400).send(errorMessage("cannot find user"))
...
async function handleAddAvatar(request: FastifyRequest, reply: FastifyReply) {
    ...
    try {
        ...
        if (!file) return reply.status(400).send(errorMessage("No file found"))
        if (!acceptFormats.includes(file.mimetype)) return reply.status(400).send(errorMessage("format not accept"))
        const user = await dbSelectUserByUsername(tokenData.username)
